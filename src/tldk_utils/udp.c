...
// 		printf("%s: netfe_put_stream %d", __func__, i + 1);
...
netfe_lcore_udp(void)
{
	...
	fe = RTE_PER_LCORE(_fe); // Generates "per_lcore__fe"
	if (fe == NULL)
		return;
	...
	if (n != 0) {
		...
		// printf("RX event received\n");
		// rte_delay_ms(10000);
	}

	/* look for tx events */
	// fs: This is the desired data of the n events returned that are UP.
	// In our case, each event contains netfe_stream objects as the data,
	// which basically means there is an event to be handled for each of
	// those n netfe_stream objects.
