...
typedef struct RTPContext {
    ...
    struct netfe_sprm *tldk_rtp_stream_prm, *tldk_rtcp_stream_prm;
...
static const AVClass tldk_rtp_class = {
    .class_name = "tldk_rtp",
...
static int rtp_open(URLContext *h, const char *uri, int flags)
{
    ...
    for (i = 0; i < max_retry_count; i++) {
        ...
        if (s->local_rtcpport < 0) {
            ...
            build_udp_url(s, buf, sizeof(buf),
                          ...
                          sources, block);
            // TODO: Refactor
            // For RTCP using TLDK UDP
            if (nspk_ffurl_open_whitelist(&s->rtcp_hd, buf, flags, &h->interrupt_callback,
                    NULL, h->protocol_whitelist, h->protocol_blacklist, h, &tldk_udp_protocol) < 0) {
        ...
        // TODO: Refactor
        // For RTCP using TLDK UDP
        if (nspk_ffurl_open_whitelist(&s->rtcp_hd, buf, rtcpflags, &h->interrupt_callback,
                    NULL, h->protocol_whitelist, h->protocol_blacklist, h, &tldk_udp_protocol) < 0)
            goto fail;
        break;
    }
    ...
    if (fec_protocol) {
        ...
        if (nspk_ffurl_open_whitelist(&s->fec_hd, buf, flags, &h->interrupt_callback, &fec_opts,
                h->protocol_whitelist, h->protocol_blacklist, h, &tldk_udp_protocol) < 0)
            goto fail;
    }
...
const URLProtocol tldk_rtp_protocol = {
    .name                      = "tldk_rtp",
    .url_open                  = rtp_open,
    .url_read                  = rtp_read,
    .url_write                 = rtp_write,
    .url_close                 = rtp_close,
    .url_get_file_handle       = rtp_get_file_handle, // Required??
    .url_get_multi_file_handle = rtp_get_multi_file_handle, // Required??
    .priv_data_size            = sizeof(RTPContext),
    .flags                     = URL_PROTOCOL_FLAG_NETWORK,
    .priv_data_class           = &tldk_rtp_class,
};
